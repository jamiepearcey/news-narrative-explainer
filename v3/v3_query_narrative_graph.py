#!/usr/bin/env python3
"""v3 query helpers for hosted-parquet narrative analysis."""
...
from v3_narrative_text_matching import asset_cues, factor_cues, match_count

ASSET_FACTOR_PRIORITY: dict[str, dict[str, float]] = json.loads(PRIORITY_CONFIG_PATH.read_text())
INDEX_ASSETS = {"NDX", "SPX"}
STRICT_GENERIC_FACTORS = {"civil_unrest", "elections_policy", "growth_activity", "labour_market"}
...
ASSET_DIRECT_CUES: dict[str, list[str]] = {
    "WTI": ["wti", "crude", "oil price", "oil prices", "brent", "barrel", "opec", "supply"],
    "US2Y": ["2-year", "two-year", "front-end", "treasury yield", "short-dated treasury", "fed path"],
    "US10Y": ["10-year", "ten-year", "treasury yield", "long-end", "term premium", "auction"],
    "DXY": ["dollar", "usd", "greenback", "dollar index", "fx"],
    "Gold": ["gold", "bullion", "xau", "precious metal", "safe haven"],
    "NDX": ["nasdaq", "nasdaq 100", "ndx", "qqq", "tech stocks", "semis"],
    "SPX": ["s&p 500", "spx", "wall street", "us stocks", "equities"],
}
FACTOR_TRANSMISSION_CUES: dict[str, list[str]] = {
    "oil": ["oil", "crude", "brent", "opec", "barrel", "supply"],
    "war_conflict": ["iran", "middle east", "hormuz", "red sea", "tanker", "shipping", "sanctions", "producer"],
    "shipping_disruption": ["shipping", "tanker", "hormuz", "red sea", "freight", "chokepoint"],
    "sanctions_trade": ["sanction", "tariff", "trade", "export", "shipping"],
    "growth_activity": ["demand", "industrial activity", "manufacturing", "consumption", "oil demand", "refinery"],
    "interest_rates": ["yield", "rates", "treasury", "fed", "policy"],
    "inflation": ["inflation", "cpi", "pce", "price pressure", "energy prices"],
}
...
def _cue_hits(text: str, cues: list[str]) -> int:
    return sum(1 for cue in cues if cue and cue.lower() in text)


def source_tier(source_domain: Any, source_type: Any = None) -> int:
    domain = str(source_domain or "").lower().strip()
    source_type_value = str(source_type or "").lower().strip()
    ...
    if domain in TIER4_SOURCE_DOMAINS:
        return 4
    if source_type_value == "commodity_specialist":
        return 2
    if source_type_value == "market_wrap":
        return 2 if domain in {"moneycontrol.com", "livemint.com", "business-standard.com"} else 3
    if source_type_value == "company_specific":
        return 4
    return 3


def _asset_factor_eligibility(doc: dict[str, Any], asset_label: str, factor_label: str | None) -> tuple[bool, str]:
    text = _normalized_text(
        doc.get("title"),
        doc.get("summary_text"),
        doc.get("market_context_text"),
        doc.get("relevant_text"),
        doc.get("body_excerpt"),
        doc.get("document_identifier"),
    )
    direct_hits = _cue_hits(text, ASSET_DIRECT_CUES.get(asset_label, []))
    factor_hits = _cue_hits(text, factor_cues(factor_label))
    transmission_hits = _cue_hits(text, FACTOR_TRANSMISSION_CUES.get(str(factor_label or ""), []))
    market_context_score = float(doc.get("market_context_score") or 0.0)
    source_type = str(doc.get("source_type") or "")

    if asset_label == "WTI":
        if factor_label in {"oil", "war_conflict", "shipping_disruption", "sanctions_trade"}:
            if direct_hits > 0:
                return True, "direct_asset"
            if transmission_hits >= 2 and _cue_hits(text, ["oil", "crude", "brent", "supply", "shipping", "tanker"]) >= 1:
                return True, "transmission_path"
            return False, "wti_missing_direct_or_transmission"
        if factor_label == "growth_activity":
            if direct_hits >= 1 and transmission_hits >= 1:
                return True, "growth_oil_link"
            return False, "growth_missing_oil_demand_link"
        if factor_label in STRICT_GENERIC_FACTORS:
            if direct_hits >= 2 and factor_hits >= 1 and (
                market_context_score >= 1.0 or source_type in {"market_wrap", "commodity_specialist"}
            ):
                return True, "generic_with_strong_direct_support"
            return False, "generic_factor_threshold_not_met"
        if direct_hits == 0:
            return False, "wti_missing_direct_cue"

    if asset_label in {"US2Y", "US10Y"} and factor_label in STRICT_GENERIC_FACTORS:
        if factor_hits == 0 and transmission_hits == 0:
            return False, "rates_generic_threshold_not_met"
        if direct_hits == 0 and market_context_score < 1.0:
            return False, "rates_missing_asset_context"

    if asset_label in INDEX_ASSETS and factor_label in STRICT_GENERIC_FACTORS:
        if factor_hits == 0 and direct_hits == 0:
            return False, "index_generic_threshold_not_met"

    if factor_label and factor_hits == 0 and transmission_hits == 0 and direct_hits == 0:
        return False, "no_factor_asset_link"
...
def _supporting_doc_relevance(doc: dict[str, Any], requested_asset_label: str) -> float:
    ...
    source_type = str(doc.get("source_type") or "")
    tier = source_tier(doc.get("source_domain"), source_type)
    source_bonus = 5.0 if tier == 1 else 3.0 if tier == 2 else -2.0 if tier == 4 else 0.5
    index_penalty = 0.0
...
def _rerank_supporting_docs(
    ...
    for row in rows:
        factor_label = requested_factor_label or str(row.get("factor_label") or "")
        keep, reason = _asset_factor_eligibility(row, requested_asset_label, factor_label)
        if not keep:
            continue
        enriched = dict(row)
    ...
    for row in eligible_rows:
        ...
        factor_label = requested_factor_label or str(rescored_row.get("factor_label") or "")
        if factor_label in STRICT_GENERIC_FACTORS:
            rescored_row["relevance_score"] = round(float(rescored_row["relevance_score"]) - 8.0, 3)
