#!/usr/bin/env python3
"""Hosted-parquet MCP wrapper for the v3 narrative explainer."""
...
from v3_narrative_text_matching import asset_cues, factor_cues, match_count, matched_cues
from v3_query_narrative_graph import (
    query_asset_narratives,
    query_asset_narratives_bulk,
    query_explain_move,
    query_supporting_docs,
    query_supporting_docs_bulk,
    run_query,
)
from v3_remote_parquet_store import DEFAULT_HTTP_ROOT, resolve_query_db


SERVER_NAME = "news-narrative-explainer-v3"
SERVER_VERSION = "0.1.0"
DEFAULT_DB = DEFAULT_HTTP_ROOT
DEFAULT_HYBRID_SEARCH_URL = "http://127.0.0.1:8788/search"
...
TIER1_SOURCE_DOMAINS = {
    "bloomberg.com", "cnbc.com", "nikkei.com", "ftchinese.com",
    "federalreserve.gov", "treasury.gov", "ecb.europa.eu", "bankofengland.co.uk",
    "opec.org", "iea.org", "imf.org", "worldbank.org",
}
TIER2_SOURCE_DOMAINS = {
    "moneycontrol.com", "livemint.com", "business-standard.com", "benzinga.com",
    "seekingalpha.com", "kitco.com", "oilprice.com", "hellenicshippingnews.com",
    "shipandbunker.com", "gcaptain.com", "rigzone.com", "worldoil.com",
    "bullionvault.com", "argusmedia.com", "financialpost.com", "afr.com",
    "borsaitaliana.it", "theglobeandmail.com", "channelnewsasia.com",
    "nasdaq.com", "morningstar.com", "investors.com", "investinglive.com",
}
TIER4_SOURCE_DOMAINS = {
    "prnewswire.com", "openpr.com", "financialcontent.com", "tickerreport.com",
}
...
def _tool_specs() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "description": "Search the corpus for a specific event or claim and rank which assets it is most likely to impact, with transmission-path evidence.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    ...
                    "candidate_assets": {"type": "array", "items": {"type": "string"}},
                    "limit": {"type": "integer", "default": 5},
                    "search_limit": {"type": "integer", "default": 8},
                    "candidate_limit": {"type": "integer", "default": 24},
                    "db": {"type": "string", "default": DEFAULT_DB},
                    "hybrid_search_url": {"type": "string", "default": DEFAULT_HYBRID_SEARCH_URL},
                    "qdrant_url": {"type": "string", "default": DEFAULT_QDRANT_URL},
                    "embedding_model": {"type": "string", "default": DEFAULT_EMBEDDING_MODEL},
                    "truncate_dim": {"type": "integer", "default": DEFAULT_TRUNCATE_DIM},
                },
                "required": ["question"],
            },
        },
...
def _source_tier_for_doc(doc: dict[str, Any]) -> int:
    domain = (_clean_text(doc.get("source_domain")) or "").lower()
    source_type = (_clean_text(doc.get("source_type")) or "").lower()
    if domain in TIER1_SOURCE_DOMAINS:
        return 1
    if domain in TIER2_SOURCE_DOMAINS:
        return 2
    if domain in TIER4_SOURCE_DOMAINS:
        return 4
    if source_type == "commodity_specialist":
        return 2
    if source_type == "market_wrap":
        return 2 if domain in {"moneycontrol.com", "livemint.com", "business-standard.com"} else 3
...
def _event_hit_to_doc(hit: dict[str, Any], query: str) -> dict[str, Any]:
    title = _clean_text(str(hit.get("title") or "")) or _clean_text(str(hit.get("document_identifier") or ""))
    summary = _clean_text(str(hit.get("summary_text") or ""))
    market_context = _clean_text(str(hit.get("market_context_text") or ""))
    theme_tags = [str(tag) for tag in hit.get("theme_tags") or [] if tag]
    relevant_text = " | ".join(part for part in [market_context, summary, " ".join(theme_tags)] if part)
    raw_score = float(hit.get("score") or hit.get("fused_score") or 0.0)
    return {
        "query": query,
        "title": title,
        "summary_text": summary,
        "market_context_text": market_context,
        "evidence_text": market_context or summary or title,
        "relevant_text": relevant_text,
        "document_identifier": _clean_text(str(hit.get("document_identifier") or "")),
        "source_domain": _clean_text(str(hit.get("source_domain") or "")),
        "source_type": _clean_text(str(hit.get("source_type") or "")),
        "partition_date": _clean_text(str(hit.get("partition_date") or "")),
        "event_time": _clean_text(str(hit.get("partition_date") or "")) or "unknown-time",
        "market_context_score": float(hit.get("market_context_score") or 0.0),
        "theme_tags": theme_tags,
        "relevance_score": raw_score,
        "classification_confidence": 0.0,
        "search_score": raw_score,
        "fused_score": float(hit.get("fused_score") or raw_score),
    }


def _payload_hits_to_event_docs(payload: dict[str, Any], query: str) -> list[dict[str, Any]]:
    docs: list[dict[str, Any]] = []
    seen: set[str] = set()
    for hit in payload.get("hits", []):
        doc = _event_hit_to_doc(hit, query)
        key = str(doc.get("document_identifier") or doc.get("title") or "")
...
def _search_event_corpus(
    *,
    question: str,
    collection: str,
    hybrid_search_url: str | None,
    qdrant_url: str,
    embedding_model: str,
    truncate_dim: int,
    limit: int,
    ...
    if hybrid_search_url:
        try:
            ...
            request = Request(
                str(hybrid_search_url),
                data=body,
                headers={"Content-Type": "application/json", "User-Agent": SERVER_NAME},
                method="POST",
            )
            with urlopen(request, timeout=60) as response:
                payload = json.loads(response.read().decode("utf-8", errors="ignore"))
            return _payload_hits_to_event_docs(payload, question)
    ...
    if proc.returncode != 0:
        ...
        if remote_error:
            detail = f"remote hybrid search failed with: {remote_error}; local fallback failed with: {detail}"
        raise RuntimeError(
            "event corpus search failed for collection "
            f"{collection!r}. {detail}"
        )
    return _payload_hits_to_event_docs(json.loads(proc.stdout), question)
...
def _factor_doc_strength(doc: dict[str, Any], asset_label: str, factor_label: str) -> dict[str, Any]:
    ...
    source_tier = _source_tier_for_doc(doc)
    score = (
        (len(asset_hits) * 3.0)
        + (len(factor_hits) * 2.0)
        ...
        + (7.0 if source_tier == 1 else 4.0 if source_tier == 2 else -6.0 if source_tier == 4 else 1.0)
    ...
    if asset_label in INDEX_ASSETS and proxy_supported:
        proxy_supported = (
            ...
            and (
                source_tier <= 2
                or doc.get("source_type") == "market_wrap"
...
def _top_factor_docs(
    ...
    if not factors:
        return {}
    pooled_docs = query_supporting_docs(
        db,
        asset_label=asset_label,
        factor_label=None,
        start_date=start_date,
        end_date=end_date,
        limit=max(len(factors) * per_factor * 6, 24),
    )
    grouped: dict[str, list[dict[str, Any]]] = {}
    for factor in factors:
        factor_label = factor["factor_label"]
        docs = [doc for doc in pooled_docs if doc.get("factor_label") == factor_label]
...
def _supported_block_rank(asset_label: str, block: dict[str, Any]) -> float:
    ...
    market_wrap_bonus = sum(
        4.0 if _source_tier_for_doc(doc) == 1 else 2.5 if _source_tier_for_doc(doc) == 2 else -3.0 if _source_tier_for_doc(doc) == 4 else 0.5
        for doc in docs
    )
...
def _source_confidence_level(
    *,
    ...
    proxy_docs: int,
    top_tier_docs: int,
    usable_finance_docs: int,
) -> str:
    evidence_bonus = direct_docs * 2 + proxy_docs
    if top_tier_docs >= 1 and unique_sources >= 2 and evidence_bonus >= 2:
        return "high"
    if usable_finance_docs >= 1 and (unique_sources >= 2 or evidence_bonus >= 1):
        return "medium"
    if market_wrap_docs >= 1 or specialist_docs >= 1 or evidence_bonus >= 1:
        return "medium"
    return "low"
...
def _source_confidence_for_docs(docs: list[dict[str, Any]]) -> str:
    ...
    top_tier_docs = sum(1 for doc in docs if _source_tier_for_doc(doc) == 1)
    usable_finance_docs = sum(1 for doc in docs if _source_tier_for_doc(doc) <= 2)
    direct_docs = sum(
        1
        for doc in docs
    ...
    return _source_confidence_level(
        ...
        top_tier_docs=top_tier_docs,
        usable_finance_docs=usable_finance_docs,
    )
...
def _source_weighting_summary_for_docs(docs: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        f"specialist_docs={specialist_docs}, top_tier_docs={top_tier_docs}, "
        f"usable_finance_docs={usable_finance_docs}, direct_docs={direct_docs}, proxy_docs={proxy_docs}"
    )
...
def _candidate_event_factors(asset_label: str) -> list[str]:
    factor_labels = set(IMPACT_RULES.get(asset_label, {}).keys())
    factor_labels.update(factor for factor, targets in TRANSMISSION_TARGETS.items() if asset_label in targets)
    ...
    return sorted(factor_labels)


def _graph_factor_context_by_asset(
    store: str,
    date: str | None,
    asset_labels: list[str],
) -> dict[str, dict[str, dict[str, Any]]]:
    if not date or not asset_labels:
        return {}
    try:
        with resolve_query_db(store, date, date) as db:
            rows = query_asset_narratives_bulk(
                db,
                asset_labels,
                start_date=date,
                end_date=date,
        ...
        return {}
    return {
        asset_label: {str(row.get("factor_label") or ""): row for row in asset_rows}
        for asset_label, asset_rows in rows.items()
    }


def _event_factor_rows_for_asset(
    asset_label: str,
    docs: list[dict[str, Any]],
    ...
    factor_docs: dict[str, list[dict[str, Any]]] = {}
    for factor_label in _candidate_event_factors(asset_label):
        scored_docs: list[dict[str, Any]] = []
        for doc in docs:
            enriched = dict(doc)
            strength = _factor_doc_strength(enriched, asset_label, factor_label)
            if not strength["supported"] and strength["score"] < 8.0:
                continue
            enriched["impact_strength"] = strength
            scored_docs.append(enriched)
        scored_docs.sort(
            key=lambda row: (
                2 if row["impact_strength"]["grade"] == "strong" else 1 if row["impact_strength"]["grade"] == "moderate" else 0,
                float(row["impact_strength"]["score"]),
        ...
        if not deduped_docs:
            continue
        factor_docs[factor_label] = deduped_docs
        support_score = sum(float(doc["impact_strength"]["score"]) for doc in deduped_docs[:2])
        row = dict(graph_context.get(factor_label) or {})
        row["factor_label"] = factor_label
        row["doc_count"] = len(deduped_docs)
        row["search_support_score"] = round(support_score, 3)
        row["adjusted_narrative_score"] = round(support_score, 3)
        row["avg_narrative_score"] = round(support_score, 3)
        factor_rows.append(row)
    factor_rows.sort(
        key=lambda row: (
            float(row.get("search_support_score") or 0.0),
            float(row.get("adjusted_narrative_score") or 0.0),
            row.get("factor_label") or "",
        ),
        reverse=True,
    )
    return factor_rows, factor_docs


def _event_asset_states(
    *,
    store: str,
    date: str | None,
    ...
    graph_context_by_asset = _graph_factor_context_by_asset(store, date, candidate_assets)
    states: list[dict[str, Any]] = []
    for asset_label in candidate_assets:
        factor_rows, factor_docs = _event_factor_rows_for_asset(
            asset_label,
            docs,
            graph_context_by_asset.get(asset_label, {}),
        )
        factor_docs = _dedupe_factor_docs(asset_label, factor_docs)
        supported_blocks, weak_labels = _supported_factor_blocks(asset_label, factor_rows[:FACTOR_CANDIDATE_LIMIT], factor_docs)
        states.append(
            {
                "asset_label": asset_label,
                "candidate_factors": factor_rows,
    ...
    states.sort(
        key=lambda state: (
            _supported_block_rank(state["asset_label"], state["supported_blocks"][0]) if state["supported_blocks"] else -1.0,
            len(state["supported_blocks"]),
            state["asset_label"],
        ),
        reverse=True,
    )
...
def _event_impact_payload(
    *,
    ...
    for state in states[:limit]:
        blocks = list(state.get("supported_blocks") or [])
        if not blocks:
            continue
        primary = blocks[0]
        factor_label = str(primary["factor"]["factor_label"])
        graph_factor_context = next(
            (
                row
                for row in state.get("candidate_factors", [])
                if row.get("factor_label") == factor_label and "asset_factor_priority_multiplier" in row
            ),
            None,
        )
        impacts.append(
            {
                ...
                "primary_factor": factor_label,
                "primary_narrative": primary.get("narrative_label") or _factor_regime_label(factor_label),
                "impact_path": _factor_impact_line(state["asset_label"], primary["factor"], primary.get("rule"))[2:],
                "support_score": primary.get("best_score"),
                "source_confidence": primary.get("source_confidence"),
                ...
                "supporting_evidence": [
                    {
                        ...
                        "strength": doc.get("impact_strength"),
                    }
                    for doc in primary.get("docs", [])[:2]
                ],
                "secondary_factors": [
                    {
                        "factor_label": block["factor"]["factor_label"],
                        "narrative_label": block.get("narrative_label"),
                        "support_score": block.get("best_score"),
                    }
    ...
    return {
        ...
        "unresolved_assets": [state["asset_label"] for state in states if not state.get("supported_blocks")][:limit],
    }
...
def _asset_state(
    ...
    factors = query_asset_narratives(
        db,
        asset_label=asset_label,
        start_date=date,
        end_date=date,
...
def _day_states(db: Path, date: str, universe: list[str], limit: int) -> list[dict[str, Any]]:
    narratives_by_asset = query_asset_narratives_bulk(
        db,
        universe,
        start_date=date,
        end_date=date,
    ...
    docs_by_asset = query_supporting_docs_bulk(
        db,
        universe,
        start_date=date,
        end_date=date,
    ...
    states: list[dict[str, Any]] = []
    for asset_label in universe:
        factors = narratives_by_asset.get(asset_label, [])[:FACTOR_CANDIDATE_LIMIT]
        pooled_docs = docs_by_asset.get(asset_label, [])
        factor_docs = {
            factor["factor_label"]: [
                doc
                for doc in pooled_docs
                if doc.get("factor_label") == factor["factor_label"]
                and (
                    (
                        float(doc.get("relevance_score") or 0.0) > 0.0
                        and _doc_mentions_asset_context(doc, asset_label)
                    )
                    or _doc_mentions_proxy_context(doc, asset_label, factor["factor_label"])
                )
            ][:2]
            for factor in factors
        }
...
def _tool_explain_move(arguments: dict[str, Any]) -> dict[str, Any]:
    store = arguments.get("db", DEFAULT_DB)
...
def _tool_query_duckdb(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    gold_scope = "window" if arguments.get("start_date") or arguments.get("end_date") else "all"
    with resolve_query_db(
        store,
...
def _tool_similar_days(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    LIMIT {limit}
    """
    with resolve_query_db(store, gold_scope="all", load_profile="gold_only") as db:
...
def _tool_assess_event_impact(arguments: dict[str, Any]) -> dict[str, Any]:
    question = str(arguments["question"]).strip()
    date = arguments.get("date")
    collection = str(arguments.get("collection") or (_default_collection_name(str(date)) if date else "")).strip()
    if not collection:
        raise ValueError("either date or collection is required for assess_event_impact")
    candidate_assets = list(arguments.get("candidate_assets") or sorted(CORE_CROSS_ASSET_SET))
    limit = max(1, min(int(arguments.get("limit", 5)), 10))
    search_limit = max(limit, min(int(arguments.get("search_limit", 8)), 20))
    candidate_limit = max(search_limit, min(int(arguments.get("candidate_limit", 24)), 100))
    docs = _search_event_corpus(
        question=question,
        collection=collection,
        hybrid_search_url=str(arguments.get("hybrid_search_url", DEFAULT_HYBRID_SEARCH_URL) or ""),
        qdrant_url=str(arguments.get("qdrant_url", DEFAULT_QDRANT_URL)),
        embedding_model=str(arguments.get("embedding_model", DEFAULT_EMBEDDING_MODEL)),
        truncate_dim=int(arguments.get("truncate_dim", DEFAULT_TRUNCATE_DIM)),
        limit=search_limit,
        candidate_limit=candidate_limit,
    )
    states = _event_asset_states(
        store=str(arguments.get("db", DEFAULT_DB)),
        date=str(date) if date else None,
        candidate_assets=candidate_assets,
        docs=docs,
    )
    return _json_content(
        _event_impact_payload(
            question=question,
            date=str(date) if date else None,
            collection=collection,
            docs=docs,
...
def call_tool(name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if name == "assess_event_impact":
        return _tool_assess_event_impact(arguments)
